...
public class Process1 extends AbstractProcess {
	...
	public void run() {
		...
		if (this.numberOfProcesses > 2) {
			send(2, message);
			message = new Message(new VectorClock(vectorClock), this.localVariable);
			monitor.receiveMessage(this.Id, message);
			
			Message receivedMessage1 = receive(2); // receive from process 1
			this.vectorClock.update(receivedMessage1.getVectorClock());
			this.localVariable = receivedMessage1.getLocalVariable() - this.localVariable;
			this.vectorClock.increment();
		}
